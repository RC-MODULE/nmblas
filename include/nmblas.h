 ...
 * \ingroup LEVEL1
 *
 * \brief вычистет параметры необходимые для пороворота Гивена 
 * 
 ...
 *
 * \details  
 * ATL_drefrotg вычисляет параметрв для поворота Гивена.<br>
 ...
 *	дальнейшего нажежного востановления c и s из одного сохраненого числа.<br>
 ...
 *
 *    if  z  = 1,    set c = 0             and s = 1,<br>
 *    if |z| < 1,    set c = sqrt(1 - z^2) and s = z,<br>
 *    if |z| > 1,    set c = 1 / z         and s = sqrt(1 - c^2).<br>
 *
 * Сотри ``Basic Linear Algebra Subprograms for Fortran Usage''  C. Law-<br>
 * son, R. Hanson, D. Kincaid and F. Krogh, ACM Transactions on Mathema-<br>
 * tical Software, 1979, 5(3) pp 308-323, для дальнейшей информации.<br>
 */ 
void nmblas_drotg(
	double *a, 
	double *b, 
...
/**
 * \ingroup LEVEL1
 * \brief функция применяет модифицированный поворот Гивена (modified-Givens rotation), матрица H предается через *PARAM
 *
 *    [ x_i ]   [ h_11 h_12 ] [ x_i ]
 *    [ y_i ] = [ h_21 h_22 ] [ y_i ]    
 *
 * Если n <= 0 или если H индентичная матрица, выход из функции происходит сразу
...
/**
 * \ingroup LEVEL1
 * \brief возвращает скалярное произведение векторов X и Y с добавлением константы B.<br>
...
/**
 * \ingroup LEVEL1
 * \brief выполняет поворот массива точек в двумерном пространстве
 * 
 * \param [in] N количество точек в массивах 
 * 
 * \param [in,out] *X указатель на массив, который содержит x координату точки 
 ...
 * \brief матрица H предается через *PARAM<br>
...
/**
 * \ingroup LEVEL 2
 * \brief выполняет следующие операцию Y = ALPHA*A[M,N]*X + BETTA*Y,
 * \brief <br> где A матрица размера M на N,ALPHA и BETTA скаляры, X и Y вектора подходящей длины 
 * 
 * \param [in] TRANS при значение 0 матрица A загружается не транспонировано, при значение 1 матрица загружается транспонировано 
 * \param [in] M количество рядов матрицы A
 * \param [in] N количество строк матрицы A
